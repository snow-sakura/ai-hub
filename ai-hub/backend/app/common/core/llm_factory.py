...
from app.common.domain.entities import ModelConfig
...
AVAILABLE_MODELS: list[ModelConfig] = [
  ...
  ModelConfig(
    provider="deepseek", model="deepseek-v4-ultra",
    display_name="DeepSeek V4 Ultra",
  ),
  ModelConfig(
    provider="deepseek", model="deepseek-v4-think",
    display_name="DeepSeek V4 Think",
  ),
  # ---- OpenAI 系列 ----
  ModelConfig(
...
# 各 provider 的默认 API Base URL（前端配置页用）
  ...
  def create(provider: str, model_name: str = "",
    ...
    if provider == "openai":
      ...
      if reasoning_effort == "disabled":
        ...
        effort = reasoning_effort
        extra_body["thinking"] = {"type": "enabled", "effort": reasoning_effort}
      return ChatOpenAI(
