...
def _validate_svg(content: bytes) -> bool:
  """验证 SVG 文件，检查是否包含 <svg 标签并过滤 XSS 风险

  安全检查：
  - 拒绝包含 <script 标签的 SVG
  - 拒绝包含 javascript: 协议的 SVG
  - 拒绝包含 onload/onclick 等事件处理器的 SVG
  """
  try:
    lower = content.lower()
    # 基础检测：必须包含 <svg 标签
    if b'<svg' not in lower:
      return False
    # XSS 防护：拒绝包含脚本或事件处理器的 SVG
    if b'<script' in lower:
      return False
    if b'javascript:' in lower:
      return False
    if b'vbscript:' in lower:
      return False
    # 拒绝常见事件处理器（全文检测）
    if re.search(rb'on\w+\s*=', lower):
      return False
    return True
  except Exception:
    return False
